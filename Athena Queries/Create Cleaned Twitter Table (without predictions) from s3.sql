CREATE TABLE "twitter_analysis"."cleaned_twitter_table_without_predictions"
WITH (
   format='TEXTFILE',
   external_location='s3://tweets-data-nl/athenaoutput/cleaned_twitter_table_without_predictions/'
) AS
select username,
    screenname,
    tweet,
    -- Find the tweet length
    length(tweet) as tweet_length,
    -- Create new column of which 1 represents that there is a retweet, and 0 being not a retweet
    CASE WHEN tweet LIKE '%RT%' THEN 1 ELSE 0 END as retweet,
    -- Extracting the username of the retweet user (minus the 'RT @')
    substr(REGEXP_EXTRACT(tweet, 'RT @[a-zA-Z0-9_.-]+'),5, length(REGEXP_EXTRACT(tweet, 'RT @[a-zA-Z0-9_.-]+'))-5+1) as retweet_users,
    followerscount,
